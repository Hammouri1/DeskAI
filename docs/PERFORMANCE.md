...
None of it opens a file, and all of it is bounded: a connect or refresh reads at most 20,000
entries eight levels deep (`SearchScanBounds.Default`, ADR 0041), a search returns at most its
query limit, and a tidy preview considers at most 500 loose files.

## How to measure

`tests/DeskAI.Presentation.Tests/PerformanceProbe.cs` generates 3,000 files (or `DESKAI_PERF_FILES`) under the
test's own temp folder and times connect, refresh, search, the Home summary, and a tidy preview. It
...
| 2026-09-24 (ADR 0041) | Same PC, Release build | 3,000 generated | 200 ms | 55 ms | 28 ms (200 shown) | 50 ms | 89 ms (500 of 500) |
| 2026-09-24 (ADR 0041) | Same PC, Release build | 20,000 generated | 1,059 ms | 306 ms | 31 ms (200 shown) | 95 ms | 116 ms (500 of 500) |
...
The 2026-09-24 runs follow the wider Search bounds. At 3,000 files the numbers are within
single-run noise of the earlier ones (the machine was busier). The 20,000-file run is a folder at
the new item bound: connecting takes about a second, a refresh about a third of a second, and
search is unchanged because it reads the index, not the disk.
