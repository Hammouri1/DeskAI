...
**Moving cards (ADR 0044).** Below the board: a caption saying DeskAI asks before the first move, or a quiet "Stop DeskAI moving things on my Desktop" link once allowed; a "Your last change stopped part-way" card when needed, with Keep them where they are and Put them back; then, right under the board, **2. Happy with these groups?** with **Put each group in its own folder** (shown only once there is a board), and after an **Other tidy-ups** heading **Clear old stuff** with Show what would move; each shows a list of tick boxes (folder or file, how many files inside, last changed or which folder it goes into, and a warning line that keeps the row unticked), the ticked total ("Ticked: 1 folder holding 2 files, plus 1 file"), Move N things, Put back with its last-change line, and Left where they are with each reason. The first Move shows the dialog "Allow DeskAI to move things on your Desktop?" with Allow moving / Cancel (Cancel is the default).
