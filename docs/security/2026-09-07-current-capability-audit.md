...
6. Recursive deletion exists only in the test-owned temporary-directory cleanup helper, which verifies both the system-temp boundary and its unique owned directory before cleanup. The product demo does not delete its temporary root or any file.
...
This result describes the current controlled-demo code, not permission for real-folder use. Real roots remain blocked until step 8 and require a new focused review. Step 7 must add write-ahead journaling, recovery, and validated undo before the organizer loop is considered recoverable.
