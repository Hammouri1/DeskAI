...
| Damaged or hostile PDF crashes or hangs Search | PdfPig runs in a separate process. Parent has a 5-second deadline, bounded pipes, and kills a stuck child. A crash or malformed response skips that file. `PlainTextExtractorTests` supplies malformed data. |
| A large or complex PDF consumes unbounded work | 8 MB per file, 20 pages, 64 KB output, 50 attempted files per search, and a 20-second search deadline checked between files. The helper also refuses input beyond 8 MB. An oversized generated file is tested. |
