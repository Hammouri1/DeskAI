...
| My workspace | Quick search card: the switch starts and stops the shortcut and is kept; seven buddies, the choice kept; another program using the shortcut is said plainly; the icon near the clock offers Find a file, and Pause only while checking; closing keeps DeskAI near the clock only while the icon really shows; Start fresh turns it back on with Sparky | `QuickSearchSettingsPageTests`, `QuickSearchLayoutTests`, `FreshStartPageTests` |
| Home, Search | Quick search tip: shown on both until closed on either, kept closed after reopening, hidden while quick search is off, back after Start fresh | `QuickSearchTipPageTests`, `QuickSearchLayoutTests` |
| Home | First-run welcome: the quick search page (Sparky and the shortcut) comes before the last page | `QuickSearchTipPageTests`, `WelcomePageTests` |
