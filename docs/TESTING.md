...
| Search | A PowerPoint file two subfolders down matches slide text only after its separate grant; result identifies Slide 2; revoking slides keeps PDF permission, and slides can be allowed without PDF; hostile XML, oversized containers, and media entries remain closed; new scopes can be disconnected | `SearchPageTests`, `PlainTextExtractorTests`, `RootCapabilitiesTests`, `SqliteAuthorizedRootRepositoryTests` |
...
| Search / Organize | Only the four and folders inside them: a picked folder outside is refused with "DeskAI only works inside your Desktop, Downloads, Documents, and Pictures." on both pages and nothing is connected; a folder inside one is accepted; a connected folder no longer inside the four cannot be allowed for tidying | `YourFoldersPageTests`, `PersonalFolderPolicyTests`, `ReadOnlyFolderServiceTests` |
