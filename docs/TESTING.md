...
| My workspace / Search | A Desktop that contains DeskAI's own protected program folder still connects and that folder's files are never remembered (owner-found bug, 2026-09-16); a folder inside the program folder is still refused as protected | `DesktopAndWallpaperPageTests`, `WindowsPathPolicyTests` |
| Whole app | No test can touch the real wallpaper or the real Desktop: both are replaced in `TestApp`, and the test Desktop is asserted to sit inside the test's own folder and differ from the real one; every test's Desktop contains a protected "program folder", as the owner's does | `DesktopAndWallpaperPageTests`, `TestApp` |
