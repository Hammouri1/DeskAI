...
| Search, Quick search | Hidden and system files (`desktop.ini`) and what is inside a hidden folder (`.git`) are not remembered, so neither Search nor the bar lists them (owner-found 2026-09-26; the tests failed before the fix) | `QuickSearchPageTests`, `MetadataIndexServiceTests` |
...
| Quick search | The bar (ADR 0047): the buddy's greeting and three examples; each row's kind tile (DOC, PDF, IMG, VID, FILE) and the selected row's Enter hint; the search icon, the Esc hint, a solid card, and an edge that turns only while buddies may move; typing finds by name, then words inside only where Search allows reading inside, never listing a file twice; up to 5 each, with See more; no folders, nothing matched, and reading-inside-not-allowed each say so plainly; Up/Down, Enter, Ctrl + Enter; the pointer over a row selects it (never a row from earlier words) and a click on the row does what Enter does, a click on its button only once (owner-found 2026-09-26); the example buttons are rounded, not ovals; the first opening on a screen with another scale (100% and 125%) is sized for that screen, and the bar refits when its scale changes (owner-found 2026-09-26); the buddy's entrance never changes its size, so it stays sharp; a program or unknown kind is only shown in its folder; a file that went away or Windows refusing keeps the bar open with the reason; each buddy speaks in its own voice; clicking the buddy; a new keystroke or hiding stops a slow inside look; nothing typed or found is remembered | `QuickSearchPageTests`, `QuickSearchServiceTests`, `QuickSearchLayoutTests` |
