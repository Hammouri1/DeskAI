...
**Desktop Studio — Find groups (ADR 0042).** The scanner now also reports each folder it comes
across as `FolderDiscovered` (entered or not), so empty folders can be listed; existing
consumers ignore it. `DesktopLookService` (`Core/Studio`) turns one bounded scan of the connected
Desktop (4 levels, 5,000 entries) into `DesktopItem`s: top-level folders with their file-ending
counts and up to 5 file names, and loose files. Hidden and system items, and any top-level folder
with a protected or link entry under it (DeskAI's own program folder), are left out; at most 60
folders and 200 files are kept, the rest returned as `LeftOutItems`. `DesktopGroupingService`
owns the flow: `PrepareAsync` numbers the items into an `AiGroupingRequest` and the exact lines
the Send window shows, sending nothing; `SendAsync` re-checks the Desktop, the AI choice, and the
sharing choices (file types, file names, and folder names for online AI), calls
`IOrganizationSuggestionProvider.GroupItemsAsync` — the connection's third method, which returns
raw text only — and `DesktopGroupReading` reads that strictly (exact shape, at most 8 groups,
names through `FolderNameCheck`, each number once) or refuses it whole. `LocalDesktopGrouper`
is DeskAI's own guess by dominant file kind, used without AI and for left-out items. Rename,
merge, and move edit the saved board. The service holds no executor, journal, writer, or setting
changer, and a test asserts that; nothing in this step changes a file or Windows.
...
`MetadataIndexService` is the only route from the filesystem into `IFileIndex`. It refuses a `Protected` root and any root `IPathPolicy` blocks, then consumes the same bounded, content-free `IFileScanner` stream and the same deterministic classifier used by planning. Hidden and system files, and everything inside a hidden or system folder (such as `desktop.ini` or a `.git` folder), are left out: File Explorer hides them, nothing DeskAI offers may act on them, and every reader of the index (Search, quick search, duplicates, the space summary) therefore leaves them out too (owner-found 2026-09-26). Each other discovered file becomes an `IndexedFile` holding a root ID, a normalized root-relative path, derived name/extension, kind, category, size, timestamps, and when it was last confirmed. Absolute paths and file content are never stored.

`IFileIndex` exposes no unscoped read: `SynchronizeRootAsync`, `ListForRootAsync`, `GetStatisticsAsync`, and `ClearRootAsync` each name one root. `SqliteFileIndex` compares the new scan against stored rows and writes only real differences, returning a `FileIndexSyncResult` of added, updated, unchanged, and removed counts so the UI can honestly say nothing changed. "Removed" means an index row was forgotten, never that a file was deleted. Rows are forgotten only after a complete look: a look that stopped at its item limit (`FileIndexLook.StoppedEarly`) adds and updates but removes nothing, because not reaching a file is no evidence it is gone (ADR 0041). The same transaction records the look in `index_looks`, which `GetStatisticsAsync` reports as `LastLookedAtUtc`, `StoppedEarly`, and `DeepFoldersSkipped`.
...
Launch search reads are bounded to 256 KB from the beginning of the file. Truncation is decided by whether bytes remain, not by whether the buffer filled, so a file of exactly the limit is complete. Extracted text is returned to the caller and stored nowhere.
...
`ContentSearchService` consumes searchable file text. It selects roots with `RootCapabilities.CanReadContent`, chooses candidates from the index by remembered name so a file DeskAI would refuse to read is never offered to the extractor, and reads only through `IContentTextExtractor`, which refuses independently. Bounds: at most 50 files per search, 256 KB of extracted text each, and phrases under three characters open nothing. Exact matching is attempted first; a local fallback ignores layout whitespace inserted inside words by PDF/Office extraction while retaining original offsets for snippets and slide evidence. The outcome carries how many files were read and whether the limit was reached, and the UI states it, so "nothing matched" is never mistaken for "nothing exists".

Flattened PDFs use a distinct route. `SearchPdfOcrAsync` is callable only after the App's
per-search confirmation and considers PDF candidates only. `IPdfOcrReader` keeps Windows
rendering/OCR out of Core; the App replaces the no-op implementation with
`WindowsPdfOcrReader`. Existing root, PDF, path, and link checks still run before bytes reach
that adapter. OCR words and page sections are returned in memory and are not indexed.
...
**2026-09-21 visual extension (ADR 0038).** Search's separate picture action calls
`VisualSearchService` only after the page's fresh read dialog. It selects candidate root IDs
from connected roots and relative paths from the metadata index. `VisualAssetReader` checks
policy, every path component for reparse points, and the opened handle's final Windows path
before returning bounded bytes. PPTX media must be referenced by a numbered slide and its
relationship; PDF images are extracted in the fixed pathless worker. The Core service holds
only a one-use, two-minute in-memory batch. `ConfiguredVisualImageMatcher` sends one image
and the typed phrase to the selected local loopback model or, after an exact-batch cloud
Send dialog, the fixed cloud provider endpoint. The response is only a bounded yes/no and
short reason; no AI output can select a path or change a file. No image index is persisted.

For the 2026-09-21 launch build, the picture-search UI entry point is removed and the
view-model capability gate is fixed off. The reviewed visual services remain dormant for
later reconsideration, but no Search-page action can reach them in this build.
