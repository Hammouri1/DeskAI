...
Index allowed metadata locally. Support structured filters first, then natural-language translation into local queries. With separate permissions, a bounded local pass searches words inside notes, modern Word and Excel files, text-based PDFs, and modern PowerPoint slides. The reviewed **Find pictures with AI** action is temporarily disabled for the launch build; rasterized page OCR and exhaustive image indexing are not claimed. Smart Collections are saved virtual queries such as University, Coding Projects, PDFs, Screenshots, and Recently Used. A Smart Collection does not move files.

The owner's broader Search goal is to ask in ordinary English and find a forgotten file inside explicitly connected folders, including nested subfolders. Examples: “Find the PDF with a picture of a dog smelling a flower” and “Find the PowerPoint with Hammouri on a slide.” Visual matching should understand the described relationship, not merely detect the separate objects. The result should name the file, show its location and the matching page or slide when known, explain whether words or visual content matched, and state what could not be searched. Deep or large folders must report any scan limits instead of implying complete coverage. PowerPoint slide text has a separate grant. The picture action asks a vision-capable AI to judge a bounded batch of images, including pictures embedded in PDFs or slides, with page or slide evidence. It is an approximate answer rather than a guarantee that every pictured word or scene will be found. The separate **Let AI read this** button still interprets only the typed sentence.
...
**Quick search** (built 2026-09-25, ADR 0047) brings finding to wherever the person is: Ctrl + Alt + D (or one of two other shortcuts the person picks on My workspace) shows a small bar with a friendly search buddy over any app. It searches the same connected folders by name, then by words inside only where Search's permission was given, and Enter opens familiar kinds of files in their usual app or shows anything else in its folder. It uses no AI, sends nothing, and remembers nothing typed.
